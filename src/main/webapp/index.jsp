...
<body onload="getOrderChart(0,0);">
	...
	<div class="row" align="center">
		...
		<div align="left">
			<div class="col-sm-2">
				<label class="control-label">时间范围：</label> <select
					class="form-control m-bot15" id="OrderChartTimeSelect"
					name="OrderChartTimeSelect" onclick="lodingChart();">
					<option value="0">最近7日</option>
		...
		<div id="chartSum0" class="align-center">
			总计：<label id="chartSum1"></label> 本
		</div>
	</div>
	...
	<script>
		function getOrderChart(timeScope, type) {
			lodingChart(timeScope, type);
		}

		// 绘制图表。
		var myChart0 = echarts.init(document.getElementById('main'));
		// 填入数据
		function lodingChart(timeScope, type) {
			timeScope = $("#OrderChartTimeSelect").val();
			var titleText = $("#OrderChartTimeSelect").find("option:selected")
					.text();
			titleText += "订单状况分析";
			type = 0;
			// 异步加载数据
			$
					.ajax({
						url : "getChartData?timeScope=" + timeScope + "&type="
								+ type,
						type : "get",
						dataType : "json",
						success : function(data) {
							document.getElementById("chartSum1").innerText = data.orderSum;
							var productTypeNameData = new Array();
							for (var i = 0; i < data.orderChartBases.length; i++) {
								productTypeNameData[i] = data.orderChartBases[i].name;
							}
							var OrderJsonArray = [];
							var OrderObject = data.orderChartBases;
							for (var i = 0; i < OrderObject.length; i++) {
								var temp = {
									value : OrderObject[i].orderNum,
									name : OrderObject[i].name
								};
								OrderJsonArray.push(temp);
							}
							// 填入数据
							myChart0.setOption({
								title : {
