...
<body>
	...
	<div class="row">
		<div class="col-lg-12">
			...
			<ol class="breadcrumb">
				...
				<li><i class="fa fa-laptop"></i>商品</li>
			</ol>
		</div>
	</div>

	<div align="left">
		<input class="form-inline col-lg-2" id="searchProductStr"
			name="searchProductStr" placeholder="商品名搜索" type="text"> <a
			class="btn btn-danger btn-sm" href="javascript:void(0)"
			onclick="searchProduct();">搜索</a> <a class="btn btn-warning btn-sm"
			href="form_add_Product">增加</a> <a class="btn btn-success btn-sm"
			href="javascript:void(0)" onclick="editProduct();">编辑</a> <a
			class="btn btn-danger btn-sm" href="javascript:void(0)"
			onclick="delProduct();">删除</a>
	</div>
	<div align="right"></div>
	<br>
	<div class="row">
			...
			<table id="ProductTable"
				...
				<thead>
					<tr>
						...
						<th><i class="icon_bag"></i> 商品名称</th>
						<th><i class="icon_book"></i> 商品类别</th>
						<th><i class="icon_image"></i> 商品图片</th>
						<th><i class="icon_archive"></i> 商品价格</th>
						<th><i class="icon_documents_alt"></i> 商品简介</th>
						<th>商品数量</th>
						<th><i class="icon_cogs"></i> 商品状态</th>
	...
	<script type="text/javascript">
		...
		function ajaxTest(num, str) {
			str = $("#searchProductStr").val();
			$
					.ajax({
						url : "ProductData?searchProductStr=" + str + "&page="
								+ num,
						type : "get",
						dataType : "json",
						success : function(data) {
							...
							for (var i = 0; i < data.data.data.length; i++) {
								...
								tablehtml += "<tr><td><label><input type='checkbox' name='selectFlag' id='checkbox' value='"+products.proId+"'/>"
										...
										+ "</td>"
										+ "<td>"
										+ "<img src='<%=path%>/upload/"+products.proImg+"' width='42' height='42'/>"
										+ "</td>"
										+ "<td>"
										+ products.proPrice
										+ "</td><td>"
										...
										+ products.proNum
										+ "</td><td>"
										+ checkDel(products.proDel)
										+ "</td>"
										+ "<td>"
										+ timeStamp2String(products.createDate)
										+ "</td></tr>";
							}
							document.getElementById("ProductTableData").innerHTML = tablehtml;
							//分页
							$("#page").paging({
								pageNo : num, //当前页
		...
		$(function() {
			$("#searchProductStr").keydown(function(e) {
				if (e.keyCode === 13) {
					searchProduct();
				}
			});
		...
		function delProduct() {
			...
			//删除
				$.ajax({
					url : "deleteProduct?selectFlags=" + index,
					type : "get",
					dataType : "json",
					success : function(data) {
						if (data.code == 0) {
							window.location.href = "form_product";
						} else {
							layer.msg(data.msg);
						}
					},
		...
		function editProduct() {
			...
			window.location = "getOneProduct?selectFlag=" + index[0];
		}

		//搜索
		function searchProduct() {
			var searchProductStr = $("#searchProductStr").val();
			ajaxTest(1, searchProductStr);
		}
	</script>
</body>
</html>
